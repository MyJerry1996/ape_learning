...
class SelfLoss(nn.Module):
	...
	def forward(self, inputs, targets):
		...
		sim_label = torch.eq(label_expand_1.view(n * n, -1), label_expand_2.view(n * n, -1)).long().view(n, n)
		...
		mean_pos_sim = sim_mat[sim_label - torch.eye(n).cuda().long()].sum() / max(1.0, (
					sim_label - torch.eye(n).cuda().long()).sum())
